<template>
    ...
    -->
	<div id="content" class="app-typecell">
		<AppNavigation>
			<AppNavigationNew v-if="!loading"
				:text="t('typecell', 'New note')"
				:disabled="false"
				button-id="new-typecell-button"
				button-class="icon-add"
				@click="newNote" />
			<ul>
				<AppNavigationItem v-for="note in notes"
					:key="note.id"
					:title="note.title ? note.title : t('typecell', 'New note')"
					...
					<template slot="actions">
						<ActionButton v-if="note.id === -1"
							...
							{{
							t('typecell', 'Cancel note creation') }}
						</ActionButton>
						<ActionButton v-else
							icon="icon-delete"
							@click="deleteNote(note)">
							{{
							 t('typecell', 'Delete note') }}
						</ActionButton>
					</template>
				</AppNavigationItem>
			</ul>
		...
		<AppContent>
			...
			<div v-else id="emptycontent">
				<div class="icon-file" />
				<h2>{{
				 t('typecell', 'Create a note to get started') }}</h2>
			</div>
		</AppContent>
	</div>
</template>
	...
	methods: {
		...
		async deleteNote(note) {
			try {
				...
				showSuccess(t('typecell', 'Note deleted'))
			} catch (e) {
				console.error(e)
				showError(t('typecell', 'Could not delete the note'))
			}
		},
	},
}
